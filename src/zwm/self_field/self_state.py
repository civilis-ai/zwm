...
class SelfState:
    ...
    def next_to_explore(self) -> int:
        ...
        """
        unvisited = [p for p in [1, 2, 3, 4, 6, 7, 8, 9]
                     if self.palace_visits.get(p, 0) == 0]
        if unvisited:
            return self._most_harmonic(unvisited)
        # 全访问过 → 选去得最少的
        counts = {p: self.palace_visits.get(p, 0) for p in range(1, 10) if p != 5}
    ...
    def record_visit(self, palace: int) -> None:
        """记录一次宫位访问."""
